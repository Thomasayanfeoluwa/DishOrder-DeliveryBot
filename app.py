...
4. **Receive updates** - We'll prepare your order
...
async def main(message: cl.Message):
    ...
    # Extract customer info from current response
    current_customer_info = extract_customer_info_from_response(response)
    
    # Update user session with current info
...
async def main(message: cl.Message):
    ...
    # [Your existing user info extraction code...]
    ...
    # Check if this is menu browsing or order discussion
    if any(keyword in response.lower() for keyword in ['menu', 'dish', 'soup', 'rice', 'chicken', 'beef']):
    ...
    # Check if final order confirmation
    if is_final_confirmation(response) and not user_sessions[user_id]['notification_sent']:
        total_amount = extract_total_amount(response)
        ...
        if total_amount > 0:
            ...
            # Get order images for manager
            order_images = image_service.get_images_for_order(response)
            
            # Send notification to manager WITH images
            notification_manager.notify_owner(
                order_details=response,
                customer_info=format_customer_info(user_sessions[user_id]),
...
⏰ Delivery time: 30-45 minutes
                ...
                await cl.Message(content="Payment system temporarily unavailable. Please try again in a few minutes.").send()
