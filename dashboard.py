...
# def extract_total_amount(response):
#     """Extract total order amount - ULTRA ROBUST VERSION"""
#     total_amount = 0.0
    
#     # Normalize the response for easier matching
#     response_lower = response.lower()
    
#     # Multiple extraction strategies
#     strategies = [
#         # Strategy 1: Look for ₦ followed by numbers
#         lambda: extract_with_patterns(response_lower, [
#             r'₦\s*([\d,]+\.?\d*)',
#             r'total.*₦\s*([\d,]+\.?\d*)',
#             r'💰.*₦\s*([\d,]+\.?\d*)',
#         ]),
        
#         # Strategy 2: Look for numbers followed by ₦
#         lambda: extract_with_patterns(response_lower, [
#             r'([\d,]+\.?\d*)\s*₦.*total',
#             r'total.*?([\d,]+\.?\d*)\s*₦',
#         ]),
        
#         # Strategy 3: Look for "total" followed by any number format
#         lambda: extract_with_patterns(response_lower, [
#             r'total.*?([\d,]+\.?\d{2})',
#             r'amount.*?([\d,]+\.?\d{2})',
#             r'final.*?([\d,]+\.?\d{2})',
...
#         # Strategy 4: Find ALL numbers and take the largest (most likely total)
#         lambda: find_largest_amount(response_lower),
#     ]
    
#     # Try each strategy until we find an amount
...
#     if total_amount == 0.0:
#         total_amount = manual_amount_extraction(response_lower)
    
#     st.sidebar.info(f"🎯 Final extracted amount: ₦{total_amount:,.2f}")
#     return total_amount
